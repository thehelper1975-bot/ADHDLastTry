...
export default function AddHabitScreen() {
  ...
  const [isLoaded, setIsLoaded] = useState(!habitId); // Only true initially if not editing
  ...
  useEffect(() => {
    const loadHabitToEdit = async () => {
      if (habitId) {
        try {
          ...
          if (json) {
             const allHabits: Habit[] = JSON.parse(json);
            ...
            console.error("Failed to load habit to edit", e);
        }
      }
      setIsLoaded(true);
    };
    ...
    loadHabitToEdit();
  }, [habitId]); // Do NOT depend on the global `habits` array
