  ...
  const updateHabit = async (habitId: string, updates: Partial<Habit>) => {
      const updated = habits.map(h => {
          if (h.id === habitId) {
             ...
             // Recalculate streak if completedDates were updated (though typically they are toggled separately)
             if (updates.completedDates) {
                updatedHabit.streak = calculateStreak(updates.completedDates);
             }
             return updatedHabit;
          }
          return h;
