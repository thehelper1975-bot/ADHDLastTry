...
export const useHabits = () => {
  ...
  const loadHabits = useCallback(async () => {
    try {
      ...
      if (json) {
        ...
        const habitsWithUpdatedStreaks = loadedHabits.map(h => ({
          ...h,
  ...
  const updateHabit = async (id: string, updates: Partial<Omit<Habit, 'id' | 'createdAt' | 'completedDates' | 'streak'>>) => {
    const updated = habits.map(h => {
      if (h.id === id) {
        return { ...h, ...updates, streak: calculateStreak(h.completedDates) };
      }
      return h;
    });
    setHabits(updated);
