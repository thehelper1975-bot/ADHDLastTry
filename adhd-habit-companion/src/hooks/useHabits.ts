  ...
  const loadHabits = useCallback(async () => {
    try {
      ...
      if (json) {
        ...
        // Recalculate streaks on load, but do not persist here to avoid infinite loops
        const updatedWithStreaks = parsed.map(h => ({
          ...h,
          streak: calculateStreak(h.completedDates)
        }));
        setHabits(updatedWithStreaks);
        habitsRef.current = updatedWithStreaks;
      } else {
        setHabits([]);
        habitsRef.current = [];
      }
