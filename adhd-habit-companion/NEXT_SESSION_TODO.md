...
- Configured app for Metro bundler on web to allow web testing (`expo export -p web`)
- Added comprehensive Habit Streak calculations using UTC dates.
- Added Habit Editing functionality to `AddHabitScreen` via `useHabits`.
- Implemented missing UX features: Edit buttons, long-press to edit, delete confirmation dialogs.
- Fixed Data synchronization across screens using `useFocusEffect` to sync local state with AsyncStorage.
