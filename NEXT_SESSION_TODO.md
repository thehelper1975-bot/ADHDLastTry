...
- **Improved App Logic:**
    - Implemented real streak calculation (daily/weekly) in `src/utils/streakCalculator.ts`.
    - Enhanced Dopamine Menu with randomized suggestions in `src/constants/dopamineMenu.ts`.
    - Added comprehensive unit tests and set up Jest environment.
    - Improved RevenueCat initialization to handle placeholders safely.
    - Added web support for verification.
...
- `adhd-habit-companion/src/utils/__tests__/streakCalculator.test.ts` (New)
...
- `adhd-habit-companion/src/contexts/SubscriptionContext.tsx`
- `adhd-habit-companion/package.json`
- `adhd-habit-companion/jest.config.js` (New)
